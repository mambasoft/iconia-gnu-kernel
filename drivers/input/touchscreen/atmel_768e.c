...
static void touch_worker(struct work_struct *work)
{
	...
	if ((mxt->PointBuf[ContactID].Status == 0) || (ContactID <= LastUpdateID) || first_touch) {
		...
		if (mxt->PointBuf[ContactID].Status != 0) {
			input_report_abs(mxt->input, ABS_X, mxt->PointBuf[ContactID].X);
			input_report_abs(mxt->input, ABS_Y, mxt->PointBuf[ContactID].Y);
			input_report_abs(mxt->input, ABS_PRESSURE, mxt->PointBuf[ContactID].Status);
		}
